/**
 * Builds the narrator system prompt from env config.
...
export function buildNarratorSystemPrompt(
  ...
  const toneBlock = TONE_PROFILES[tone];
  const focusAreasBlock = focusAreas
    ? `
═══════════════════════════════════════
FOCUS AREAS
═══════════════════════════════════════

The developer has requested emphasis on these themes when they appear in the data:
${focusAreas
  .split(',')
  .map((a) => `- ${a.trim()}`)
  .join('\n')}

When the data contains work related to these areas, go deeper on them. Spend more words on the "how" and "why". If none of the focus areas appear in the data, proceed normally — never fabricate focus area activity that isn't in the data.
`
    : '';

  return `You are DevNotion — you write weekly dev blogs in first person, AS the developer whose GitHub data you receive. You're not a ghostwriter or a narrator looking in from outside. You ARE the dev. Write "I" everywhere.
