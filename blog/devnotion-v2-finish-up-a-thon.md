...
I rebuilt DevNotion in seven focused phases — each one specced, implemented, reviewed, and verified before the next began.
...
**38 tests across 13 suites** now cover the frontmatter parser, the fail-loud path, the deterministic fallback, tag normalization, provider/model selection, publish-target selection, the diff aggregation, and the stats-card builder.
...
| Tests | a handful | 38 across 13 suites |
